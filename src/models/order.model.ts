...
export interface PortfolioAsset {
  symbol: string; // Asset identifier (AAPL, BTC, GOLD, etc.)
  type?: AssetType; // Asset type (optional, for categorization)
  allocation?: number; // Percentage (0-100) - for model portfolios
  amount?: number; // Dollar amount - for manual investments
  price?: number; // Optional custom price, defaults to $100
}
...
export interface AssetOrder {
  ...
  amount: number; // Dollar amount allocated
  shares: number; // Number of shares/units (with decimal precision)
  totalCost: number; // Actual cost (shares * price)
}
...
export interface Order {
  ...
  nextAvailableTime?: string; // ISO 8601 format if market is closed
  createdAt: string;
  ...
  metadata: {
    ...
    mode: 'allocation' | 'amount'; // Which input mode was used
    assetBreakdown?: Record<AssetType, number>; // Count by asset type
  };
}
...
export interface CreateOrderRequest {
  orderType: OrderType;
  amount?: number; // Optional for allocation-based, omit for amount-based
  portfolio: ModelPortfolio;
}
