...
import { Order, OrderQueryParams, PaginatedOrders, AssetType } from '../models/order.model';
...
export class OrderRepository {
  ...
  private ordersByType: Map<string, Set<string>>;
  private ordersBySymbol: Map<string, Set<string>>;
  private ordersByAssetType: Map<AssetType, Set<string>>;
  
  // Ordered index for pagination - maintains insertion order
  private ordersByDate: string[];
  ...
  private constructor() {
    ...
    logger.info('OrderRepository initialized with high-performance indexing', {
  ...
  save(order: Order): Order {
    ...
    if (duration > 10) {
      logger.warn('Slow order save operation', {
  ...
  findAll(params: OrderQueryParams): PaginatedOrders {
    ...
    if (params.orderType) {
      ...
      if (typeIds) filters.push(typeIds);
      else return this.emptyResult(params);
    }
    
    // Apply symbol filter
    if (params.symbol) {
      const symbolIds = this.ordersBySymbol.get(params.symbol);
      if (symbolIds) filters.push(symbolIds);
      else return this.emptyResult(params);
    }
    ...
    if (params.assetType) {
      ...
      if (assetTypeIds) filters.push(assetTypeIds);
      else return this.emptyResult(params);
    }
  ...
  getStats(): {
    ...
    const ordersByAssetType: Record<string, number> = {};
  ...
  clear(): void {
    ...
    this.ordersByDate = [];
    this.statsCache = null;
    logger.info('All orders cleared from repository');
  }
   ...
   */
  private filterByDateRange(orderIds: string[], fromDate?: string, toDate?: string): string[] {
    if (!fromDate && !toDate) return orderIds;
   ...
   */
  private sortByDate(orderIds: string[], direction: 'asc' | 'desc'): string[] {
   ...
   */
  private batchGetOrders(orderIds: string[]): Order[] {
    const orders: Order[] = [];
   ...
   */
  private emptyResult(params: OrderQueryParams): PaginatedOrders {
    return {
      orders: [],
      pagination: {
        total: 0,
